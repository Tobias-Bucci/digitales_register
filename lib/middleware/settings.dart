...
Future<void> _applyDetectedSubstituteTeacherSettings(
  ...
  for (final subject in api.state.extractAllSubjects()) {
    ...
    mergedPrimaryTeachers[resolvedSubject] =
        detectedTeacher == null ? const <String>[] : <String>[detectedTeacher];
  }
...
Future<void> _ensureSubstituteTeacherHistoryLoaded(
  ...
  if (api.state.calendarState.prefetchedSchoolYears.contains(schoolYear) &&
      _hasCompletedInitialSubstituteTeacherSetup(api.state)) {
    return;
  }
  final sessionKey = _substituteTeacherHistorySessionKey(api.state);
  if (_substituteTeacherHistoryLoadingKeys.contains(sessionKey)) {
  ...
  await _substituteTeacherHistoryMutex.protect(() async {
    ...
    try {
      ...
      // Keep automatic teacher detection suspended while all school-year
      // weeks are arriving. Otherwise the first loaded week could be mistaken
      // for the stable teacher baseline. Detection and substitute marking are
      // applied once, using the complete locally cached year.
      await _ensureSchoolYearCalendarLoaded(api);
      if (api.state.calendarState.prefetchedSchoolYears.contains(schoolYear)) {
        ...
        await _applyDetectedSubstituteTeacherSettings(api);
        await api.actions.calendarActions.recalculateSubstitutes(
          _currentSubstituteDetectionConfig(api.state),
        );
      ...
      final anchorMonday =
          api.state.calendarState.currentMonday ?? toMonday(now);
...
bool _hasCompletedInitialSubstituteTeacherSetup(AppState state) {
  return state.settingsState.subjectNicksAutoGenerated;
}

@visibleForTesting
bool shouldPrefetchSubstituteTeacherHistory(AppState state) {
  return _shouldPrefetchSubstituteTeacherHistory(state);
}
...
bool _shouldPrefetchSubstituteTeacherHistory(AppState state) {
  ...
  if (_hasCompletedInitialSubstituteTeacherSetup(state)) {
    return false;
  }
